...
def generate_residential_inflexible_loads(env) -> np.ndarray:
    ...
    # 根据 sim_date 计算在 731 天大文件中的起始偏移量
    start_idx = get_data_offset(env)
    
    # 关键校验：确保数据行数足够从 start_idx 开始读取 simulation_length 长度
    if len(env.load_data) < start_idx + env.simulation_length:
        print(f"Warning: Load data file length {len(env.load_data)} is not enough for start_idx {start_idx}. Resetting start_idx to 0.")
        start_idx = 0
    ...
    for i in range(number_of_transformers):
        for j in range(simulation_length):
            # 从偏移量位置开始读取
            if env.load_data[start_idx + j] > 0:
                inflexible_loads[i, j] = env.load_data[start_idx + j]
            else:
...
def generate_pv_generation(env) -> np.ndarray:
    ...
    # 关键校验：确保PV数据行数足够从 start_idx 开始读取 simulation_length 长度
    if len(env.pv_data) < start_idx + env.simulation_length:
        print(f"Warning: PV data file length {len(env.pv_data)} is not enough for start_idx {start_idx}. Resetting start_idx to 0.")
        start_idx = 0
    ...
    for i in range(number_of_transformers):
        # 为每个变压器生成独立的随机系数，从偏移量位置读取基础PV数据
        random_factor = env.tr_rng.uniform(0.9, 1.1)
        pv_generation[i, :] = env.pv_data[start_idx : start_idx + simulation_length] * random_factor

    # 返回格式：(变压器数量, 仿真时长)的numpy数组
    return pv_generation

def update_transformer_data(env):
    '''
    根据当前 sim_date 更新现有变压器的负荷和光伏数据
    '''
    if not env.config['inflexible_loads']['include'] and not env.config['solar_power']['include']:
        return
    ...
    if env.config['inflexible_loads']['include']:
        ...
        if len(env.load_data) >= start_idx + env.simulation_length:
            for i in range(env.number_of_transformers):
                env.transformers[i].inflexible_load = env.load_data[start_idx : start_idx + env.simulation_length]
        else:
            print(f"Warning: Load data length not enough for offset {start_idx}")

    if env.config['solar_power']['include']:
        if not hasattr(env, 'pv_data') or env.pv_data is None:
            pv_df = pd.read_csv(data_path, sep=',', header=0, usecols=['pv'])
            env.pv_data = pv_df['pv'].values
        
        if len(env.pv_data) >= start_idx + env.simulation_length:
            for i in range(env.number_of_transformers):
                random_factor = env.tr_rng.uniform(0.9, 1.1)
                env.transformers[i].solar_power = env.pv_data[start_idx : start_idx + env.simulation_length] * random_factor
        else:
            print(f"Warning: PV data length not enough for offset {start_idx}")
    ...
    for tr in env.transformers:
        ...
        tr.reset(step=env.current_step)
...
def get_data_offset(env):
    '''
    计算当前 sim_date 在 731 天数据集中的起始行索引
    假设数据集从 2022-01-01 开始，每天 96 步
    '''
    start_date = datetime(2022, 1, 1)
    # 计算天数差
    days_diff = (env.sim_date.replace(hour=0, minute=0, second=0, microsecond=0) - start_date).days
    # 确保在 0-730 天范围内（防溢出）
    days_diff = max(0, min(days_diff, 730))
    # 每步对应 96 个时间步（假设 timescale=15min）
    # 如果 timescale 不同，这里需要根据 env.simulation_length 动态计算
    # 但根据用户描述，每天是 96 步
    return days_diff * 96
...
def load_electricity_prices(env) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # 校验数据长度是否足够从 start_idx 开始读取 simulation_length 长度
    if len(env.price_data) < start_idx + env.simulation_length:
        print(f"Warning: Data file length {len(env.price_data)} is not enough for start_idx {start_idx} and simulation_length {env.simulation_length}. Resetting start_idx to 0.")
        start_idx = 0
    ...
    for i in range(env.simulation_length):
        # 从偏移量位置开始读取
        base_price = env.price_data[start_idx + i]
