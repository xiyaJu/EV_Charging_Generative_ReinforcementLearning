...
Key changes vs original:
  ...
  - write cv_summary.json with per-fold best validation losses
  - when generate_days>0 after CV training, generation uses the best fold model by default

Notes:
  - if --model_dir is provided, it should point to a concrete fold directory containing model.pt,
    or to a normal non-CV run directory containing model.pt.
"""
from __future__ import annotations
import argparse, json, datetime, uuid, csv
...
def train_one_fold(
    ...
    X: np.ndarray,
    C: np.ndarray,
    day_load_sum: np.ndarray,
    day_lambda_max: np.ndarray,
    x_dim: int,
    c_dim: int,
    ...
    model = MLP_CVAE(x_dim=x_dim, c_dim=c_dim, hidden_dims=hidden_dims, latent_dim=latent_dim).to(device)
    opt = torch.optim.Adam(model.parameters(), lr=args.lr)

    sampler = None
    if args.extreme_metric != "none":
        if args.extreme_metric == "load_sum" and len(day_load_sum) == len(X):
            score = day_load_sum
        elif args.extreme_metric == "lambda_max" and len(day_lambda_max) == len(X):
        ...
        print(f"[Fold {fold_no}] [Extreme sampler] metric={args.extreme_metric} top_pct={args.extreme_top_pct} thr={thr:.4f} weight={args.extreme_weight}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch == 1 or epoch % 10 == 0 or epoch == args.epochs:
            print(
                f"[Fold {fold_no}] Epoch {epoch:4d}/{args.epochs} | train {tr_loss:.6f} (r={tr_rec:.6f},kl={tr_kl:.6f}) | "
    ...
    save_json(
        ...
        str(fold_dir / "config.json"),
    )
    save_json(history, str(fold_dir / "history.json"))
    print(f"[OK] saved fold {fold_no} in {fold_dir}")
...
def main():
    ...
    if not args.model_dir.strip():
        if args.cv_folds > 1:
            ...
            all_folds = [split_indices(N, args.val_ratio, args.seed)]
        ...
        for fold_no in fold_ids:
            train_idx, val_idx = all_folds[fold_no - 1]
            rec = train_one_fold(
                fold_no=fold_no,
                train_idx=train_idx,
                val_idx=val_idx,
                ...
                X=X,
                C=C,
                day_load_sum=day_load_sum,
                day_lambda_max=day_lambda_max,
                x_dim=x_dim,
                c_dim=c_dim,
                hidden_dims=hidden_dims,
                latent_dim=latent_dim,
        ...
        if args.cv_folds > 1:
            losses = np.array([r["best_val_loss"] for r in cv_records], dtype=np.float64)
            best_idx = int(np.argmin(losses))

            metrics_csv_path = run_dir / "cv_fold_metrics.csv"
            with open(metrics_csv_path, "w", newline="", encoding="utf-8") as f:
                writer = csv.DictWriter(f, fieldnames=["fold", "train_size", "val_size", "best_val_loss", "fold_dir"])
                writer.writeheader()
                for r in cv_records:
                    writer.writerow({
                        "fold": int(r["fold"]),
            ...
            print(f"[CV] mean_best_val_loss={summary['mean_best_val_loss']:.6f} std={summary['std_best_val_loss']:.6f}")
            selected_model_dir = Path(summary["best_fold_dir"])
        else:
            selected_model_dir = run_dir

    if args.generate_days > 0:
        if not (Path(selected_model_dir) / "model.pt").exists():
            raise FileNotFoundError(f"No model.pt found under generation source dir: {selected_model_dir}")

        model = MLP_CVAE(x_dim=x_dim, c_dim=c_dim, hidden_dims=hidden_dims, latent_dim=latent_dim)
        model.load_state_dict(torch.load(str(Path(selected_model_dir) / "model.pt"), map_location=device))
        model.to(device)
        model.eval()
        ...
        gen_dir = make_gen_dir(Path(selected_model_dir), gen_tag)
            ...
            meta=np.array(
                {
                    ...
                    "model_dir_used": str(selected_model_dir),
                    "price_params": {
